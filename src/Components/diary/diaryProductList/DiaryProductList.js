...
const DiaryProductList = () => {
  ...
  return (
    <>
      {loading ? (
        ...
        <ul className={styles.diaryProductList}>
          {eatenProducts?.length === 0 ? (
            <span className={styles.emptyDiary}>Съеденные в этот день продукты ещё не добавлены</span>
          ) : (
            eatenProducts?.map(item => (
              <li key={item.id} className={styles.diaryProductListItem}>
                ...
                <button id={item.id} className={styles.diaryProductListButton} onClick={handleDelete}>
                  ×
                </button>
              </li>
            ))
      ...
      {size.width < 768 ? (
        <button type="button" className={styles.diaryButtonRounded} onClick={toggle}>
          +
        </button>
      ) : null}
    </>
