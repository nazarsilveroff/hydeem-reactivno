...
import { getDaySummarySelector, getInfoForDaySelector, getSelectedDayString } from "../../redux/day/daySelectors";
        ...
        <>
          <div className={style.container}>
            <div className={style.summary}>
              <h2 className={style.title}>Сводка за {daySummary?.date || selectedDay}</h2>
              <div className={style.statistics}>
                ...
                <ul className={style.list}>
                  <li className={style.text}>{Math.round(defaultData?.kcalLeft || daySummary?.kcalLeft) || 0}</li>
                  <li className={style.text}>{Math.round(daySummary?.kcalConsumed) || 0}</li>
                  <li className={style.text}>{Math.round(defaultData?.dailyRate || daySummary?.dailyRate) || 0}</li>
                  <li className={style.text}>{Math.round(daySummary?.percentsOfDailyRate) || 0}%</li>
                </ul>
              </div>
            </div>
            <div className={style.meny}>
              ...
              <div className={style.products}>
                <ul className={style.defaultText}>
                  {notAllProd?.length > 0
                    ? notAllProd?.slice(0, 9).map(item => <li key={item}>{item}</li>)
