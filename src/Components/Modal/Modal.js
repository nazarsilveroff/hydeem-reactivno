...
const Modal = () => {
  ...
  const filteredNotAllowedProducts = () => {
    return notAllowedProductsList
      ?.slice(0, 20)
      .filter((filteredProduct) =>
        filteredProduct.toLowerCase().includes(search)
      );
  };
                    ...
                    {filteredNotAllowedProducts()?.length ? (
                      <ol className={styles.forbiddenProductsList}>
                        {filteredNotAllowedProducts()?.map((product) => (
                          <li
                            className={styles.forbiddenProductsItems}
                            key={product}
                          >
                            {product}
                          </li>
                      ...
                      <span className={styles.noResultMessage}>
                        Нет результата на данный поиск
                      </span>
                    )}
                  </div>
                  <Link
                    type="button"
                    to="/login"
                    className={styles.startGettingSlimButton}
                    onClick={toggle}
                  >
                    Начать худеть
                  </Link>
